...
**현재 상태 (전면 표준화 Phase 0 완료 시점)**

| 명령           | 기대 결과 | 비고                                   |
| -------------- | --------- | -------------------------------------- |
| `format:check` | 통과      | 반드시 통과해야 한다                   |
| `lint`         | **실패**  | 기존 위반 131건. Phase 1에서 해소      |
| `test:ci`      | **실패**  | 커버리지 임계치 미달. Phase 1에서 해소 |
| `build`        | 통과      | 반드시 통과해야 한다                   |

`lint`·`test:ci`가 실패하는 것은 현재 정상이다. 중요한 것은 **내 변경이 위반 수를 늘리지
않는 것**이다. 기준선: `docs/superpowers/plans/phase0-lint-baseline.txt`
...
| 1     | 인증/API 레이어 통일 및 레거시 제거    | 예정     |
| 2     | 게임 라우팅 및 진행 상태 정합성 수정   | 예정     |
