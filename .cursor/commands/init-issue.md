...
**형식**: `[날짜]_[#일감번호]_[일감제목_대괄호제거]`

**상세 규칙**
1. **날짜**: `YYYYMMDD` 형식 (오늘 날짜 사용)
2. **일감 번호**: `#` 포함하여 표기 (예: `#202027`)
3. **일감 제목 변환**:
   - **대괄호 `[]` 내용 완전 제거** (예: `[CM]`, `[CM2.0]`, `[XDP 고도화]` 모두 제거)
   - 소괄호 `()` 는 유지
   - 공백은 언더스코어 `_`로 변환
   - 슬래시 `/` 는 언더스코어로 변환
   - 특수문자 제거: `:`, `,`, `.`, `?`, `!` 등
...
출력: 20251013_#202027_MAToolDaemon.jar_기동_시_crontab_SMSScheduler_라인_제거_개선_요청의_건
```

**예시 2**
```
입력: 시험요청 #198622
      [CM2.0][XDP 고도화] Multi Process UI 추가 요청
      날짜: 2025년 10월 27일

출력: 20251027_#198622_Multi_Process_UI_추가_요청
```
...
3. 내부 파일 수정:
   - `new_템플릿_글꼴포함.pptx` → `[브랜치명].pptx` 로 파일명 변경
   - 나머지 파일은 원본 그대로 유지
...
**⚠️ 주의**: Python의 `write()` 도구 또는 `open()` 함수를 사용하여 UTF-8로 저장합니다.
...
**파일 작성 방법**:
```python
# Cursor의 write 도구 사용 (권장)
write_file(
    f"{branch_folder_path}\\B-브랜치, 커밋 제목.txt",
    content,
    encoding='utf-8'
)
...
**커밋 메시지 생성 규칙**
- **형식**: `[일감제목_대괄호제거] : [타입] : [변경사항 설명] http://redmine.somansa.com/redmine/issues/[일감번호]`
- **타입 판단**:
  - 신규 기능: `feat`
  - 버그 수정/개선: `fix`
  - 리팩토링: `refactor`
  - 문서: `docs`
- **일감 제목**: **대괄호 `[]` 내용 완전 제거** (예: `[CM]`, `[CM2.0]` 모두 제거)
...
20251013_#202027_MAToolDaemon.jar_기동_시_crontab_SMSScheduler_라인_제거_개선_요청의_건

2. 커밋 메시지

MAToolDaemon.jar 기동 시 crontab SMSScheduler 라인 제거 개선 요청의 건 : feat : root와 somansa 계정 crontab SMSScheduler 중복 라인 자동 제거 기능 추가 http://redmine.somansa.com/redmine/issues/202027
...
20251027_#198622_Multi_Process_UI_추가_요청

2. 커밋 메시지

Multi Process UI 추가 요청 : feat : XDP Multi Process 모드(Single/Active-Active/Active-Standby) 설정 UI 및 상태 모니터링 기능 추가 http://redmine.somansa.com/redmine/issues/198622

3. 기능정의서 문서 제목

FD-PLAB-20251027-198622: [CM2.0][XDP 고도화] Multi Process UI 추가 요청

4. 테스트케이스 문서 제목

DTR-PLAB-20251027-198622: [CM2.0][XDP 고도화] Multi Process UI 추가 요청 - 개발자 테스트
...
### ✅ 해결 방법: common_util.py 사용

**`.cursor/scripts/common_util.py` 모듈을 사용하여 한글 경로 문제를 해결합니다.**
...
dest = r"d:\\0-suh\\document\\{branch_name}"

# 폴더 복사 (자동 검증 포함)
result = copy_folder(source, dest, overwrite=True, verify=True)

if result["success"]:
    print(f"[SUCCESS] 폴더 복사 완료: {{result['dest_path']}}")
    print(f"[INFO] 파일 개수: {{result['file_count']}}개")
else:
    print(f"[ERROR] 복사 실패: {{result['error']}}")
'''
...
**동작 과정**:
1. 파싱된 브랜치명을 변수에 저장
2. `common_util.copy_folder()` 함수를 사용하는 Python 스크립트 생성
3. 스크립트 실행 (UTF-8 인코딩으로 한글 경로 처리)
4. 7개 파일 복사 및 검증
5. **PPT 파일명 변경**: `new_템플릿_글꼴포함.pptx` → `[브랜치명].pptx`
6. `B-브랜치, 커밋 제목.txt` 파일 작성
7. 임시 스크립트 삭제

**주요 함수**:
- `copy_folder(src, dest, overwrite=True, verify=True)`: 폴더 복사 및 자동 검증
- `os.rename(old_path, new_path)`: 파일명 변경
...
- **폴더 복사 완전성**: 템플릿 폴더의 **모든 파일**을 빠짐없이 복사
  ...
  - C-일감 보고 텍스트 템플릿.txt
  - 테스트코드_템플릿.txt
  - B-브랜치, 커밋 제목.txt (내용 수정)
  - A-개발보고서, 테스트보고서 URL.txt
- **파일 수정 사항**:
  - `B-브랜치, 커밋 제목.txt`: 내용 자동 작성
  - `new_템플릿_글꼴포함.pptx`: 파일명을 `[브랜치명].pptx`로 변경
  - 나머지 파일: 원본 그대로 유지
- **브랜치명 생성 규칙**: 일감 제목에서 **대괄호 `[]` 내용 완전 제거**
- **커밋 메시지 규칙**: 일감 제목에서 **대괄호 `[]` 내용 완전 제거**
- **기능정의서/테스트케이스**: 일감 제목 **원본 유지** (대괄호 포함)
